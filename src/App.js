...
export default function LeverageCalculator() {
  ...
  const results = [
    ...
    { label: "SL Price", value: `${slPrice.toFixed(2)} USDT` },
    {
      ...
      value: `${liquidation.toFixed(2)} USDT — ${
        slSafe ? "SL is Safe ✅" : "SL below liquidation ❌"
      }`,
    },
      ...
      <div className={`max-w-xl mx-auto rounded-xl shadow-md p-6 transition-colors duration-300 ${darkMode ? "bg-gray-800 text-gray-100" : "bg-white text-gray-900"}`}>
        <div className="flex justify-between items-center mb-4">
          ...
          <button
            ...
            className={`text-sm px-3 py-1 border rounded transition-colors ${darkMode ? "hover:bg-gray-700" : "hover:bg-gray-200"}`}
          >
            {darkMode ? "☀️" : "🌙"}
          </button>
        </div>
        ...
        <div className="space-y-4">
          ...
          <button
            className="w-full mt-2.bg-blue-600 hover:bg-blue-700 text-white py-2 px-4.rounded disabled:opacity-50"
        ...
        {calculated && isValid && (
          <div className={`mt-6 p-4.rounded border.space-y-3 ${darkMode?"bg-gray-700.border-gray-600":"bg-gray-100.border-gray-300"}`}>
            <div className="flex.justify-between items-center">
              <h3 className="text-xl.font-semibold">Results</h3>
              <button onClick={copyAllResults} className="text-xs.text-blue-400 hover:text-blue-600">Copy All</button>
            </>
Explaination truncated due to size constraints; use earlier full App.js content.
